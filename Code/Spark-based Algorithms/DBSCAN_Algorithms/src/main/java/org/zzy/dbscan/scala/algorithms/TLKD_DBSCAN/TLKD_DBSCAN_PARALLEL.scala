...
//先使用sample-KD树进行数据分区，然后使用基于two-level KD树的DBSCAN进行局部聚类，最后联通
object TLKD_DBSCAN_PARALLEL extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    conf.setAppName("TLKD_DBSCAN_PARALLEL")
      .setMaster(master)
...
//    println("本地聚类之前时间："+System.currentTimeMillis())
    ...
    val clustered =
      duplicated
        ...
        new TLKD_DBSCAN.Clustering(points,eps,minpts).fit())
        .cache()
