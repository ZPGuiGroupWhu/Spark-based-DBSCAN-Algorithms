package org.zzy.dbscan.scala.algorithms.GRID

import org.apache.spark.{SparkConf, SparkContext}
import org.slf4j.LoggerFactory
import org.zzy.dbscan.java.TLKD.process
import org.zzy.dbscan.java.index.Grid.{GridIndex, GridIndexHashAll}
...
//          .setMaster(master)
  ...
  class KDBSCAN(points:Iterable[KDBSCANPoint],eps: Double,minPoints:Int)extends Serializable {
    ...
    val x=((maxX-minX)/eps+1)*eps+minX
    val y=((maxY-minY)/eps+1)*eps+minY

    val rectange=new process().getRectange(minX,minY,x,y)
    val gridIndexAll= new GridIndexHashAll().build(rectange,eps,eps)
    new GridIndex().creatPointGridIndex(points.toList.asJava,gridIndexAll)

    def fit():Iterable[KDBSCANPoint] ={
      val labeledPoints=gridIndexAll.getPoints.asScala.toArray
      val totalClusters=labeledPoints.foldLeft(0)((cluster,point)=>{
        if (!point.isVisited) { //点没有访问则继续往下
          point.setVisited(true)
          val neighbors = gridIndexAll.rangeQuery2(point,eps).asScala.toIterable
      ...
      while (allNeighbors.nonEmpty) {
        ...
        allNeighbors.dequeue().foreach(neighbor => {
          if (!neighbor.isVisited) {//如果点没有访问过
            ...
            val neighborNeighbors = gridIndexAll.rangeQuery2(neighbor,eps).asScala.toIterable
